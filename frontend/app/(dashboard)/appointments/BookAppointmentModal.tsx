...
export function BookAppointmentModal({ onClose }: Props) {
  ...
  const [form, setForm] = useState({ doctorId: "", scheduledAt: "", reason: "", notes: "" });
  const [selectedDate, setSelectedDate] = useState(format(new Date(), "yyyy-MM-dd"));

  const { data: doctorsData } = useGetAllDoctorsQuery({ search, limit: 20 });
  const doctors = doctorsData?.data || [];
  const selectedDoctor = doctors.find((d: any) => d.id === form.doctorId || d.userId === form.doctorId);

  const { data: slots } = useGetAvailableSlotsQuery(
    { doctorId: form.doctorId, date: selectedDate },
    { skip: !form.doctorId || !selectedDate }
  );
      ...
      <div className="bg-card rounded-2xl border shadow-xl w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {doctors.map((d: any) => (
                  <button key={d.id} type="button"
                    onClick={() => { setForm({ ...form, doctorId: d.userId || d.id }); setStep(2); }}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">{d.specialty} · {d.department?.name}</p>
                    </div>
                  </button>
                ))}
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {slots.map((slot: string) => (
                      <button key={slot} type="button"
                        onClick={() => setForm({ ...form, scheduledAt: slot })}
                        className={`py-1.5 text-xs rounded-lg border transition font-medium ${
                          form.scheduledAt === slot ? "bg-helix-600 text-white border-helix-600" : "hover:border-helix-400"
                        }`}>
          ...
          <div className="flex gap-3 pt-2">
            ...
            {step === 2 && (
              <button type="submit" disabled={isLoading || !form.reason || !form.scheduledAt}
